...
First read CLAUDE.md in the current directory to find the project vault folder.
Then based on the topic requested, read and summarize the relevant note from that vault folder:
...
- "portfolio" → read the project's Portfolio/ note if it exists

After reading, output:
"📖 [note name] loaded" followed by a concise summary of the most relevant parts.

If topic is not recognized, list all available topics based on what actually exists in the project vault folder.
